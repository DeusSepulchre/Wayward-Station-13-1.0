...
/datum/game_mode/dynamic
	...
	/// List of event rules for selecting midround events
	var/list/event_rules = list()
	...
	///When world.time is over this number the mode tries to inject an event
	var/event_injection_cooldown = 0
	///When world.time is over this, increase the threat level
	var/threat_injection_cooldown = 0
...
/datum/game_mode/dynamic/Topic(href, href_list)
	...
	else if (href_list["injectevent"])
		event_injection_cooldown = 0
		message_admins("[key_name(usr)] forced an event injection.", 1)
	else if (href_list["injectthreat"])
		threat_injection_cooldown = 0
		message_admins("[key_name(usr)] forced a threat injection.", 1)
...
/datum/game_mode/dynamic/send_intercept()
	...
	var/starting_threat_level = (GLOB.dynamic_chaos_level * 20)
	switch(round(starting_threat_level))
		if(0 to 19)
			update_playercounts()
			if(!current_players[CURRENT_LIVING_ANTAGS].len)
				. += "<b>Peaceful Waypoint</b></center><BR>"
				. += "Your station orbits deep within controlled, core-sector systems and serves as a waypoint for routine traffic through friendly Nanotrasen space. Due to the combination of high security, interstellar traffic, and low strategic value, it makes any direct threat of violence unlikely. Your primary enemies will be incompetence and bored crewmen: try to organize team-building events to keep staffers interested and productive."
			else
		...
		if(40 to 49)
			...
			. += "Your station's orbit passes along the edge of Nanotrasen allied influence. While subversive elements remain the most likely threat against your station, Nanotrasen and hostile organizations are bolder here, where our grip is weaker. Exercise increased caution against potential Syndicate agents, and be on the lookout for any potentially anomalous activity."
		if(50 to 64)
			. += "<b>Uncharted Space</b></center><BR>"
			. += "Congratulations and thank you for participating in the Nanotrasen 'Frontier' space program! Your station is actively orbiting a high value system far from the nearest support stations. Little is known about your region of space, and the opportunity to encounter the unknown invites greater glory. You are encouraged to elevate security as necessary to protect your lives and station assets."
...
/datum/game_mode/dynamic/proc/generate_threat() //Generates initial threat.
/*
...
/datum/game_mode/dynamic/proc/picking_roundstart_rule(list/drafted_rules = list(), forced = FALSE)
	...
	if (starting_rule.pre_execute())
		var/rule_threat = starting_rule.get_threat(roundstart_pop_ready)
		spend_threat(rule_threat)
		spend_threat(starting_rule.cost)
		threat_log += "[worldtime2text()]: Roundstart [starting_rule.name] spent [starting_rule.cost + rule_threat]"
...
/datum/game_mode/dynamic/proc/picking_midround_latejoin_rule(list/drafted_rules = list(), forced = FALSE)
	...
	if(!rule.repeatable)
		if(rule.ruletype == "Latejoin")
			...
			event_rules = remove_from_list(event_rules, rule.type)

	if (rule.execute())
		log_game("DYNAMIC: Injected a [rule.ruletype == "latejoin" ? "latejoin" : "midround"] ruleset [rule.name].")
		spend_threat(rule.cost)
		var/rule_threat = rule.get_threat(current_players[CURRENT_LIVING_PLAYERS].len)
		spend_threat(rule_threat)
		threat_log += "[worldtime2text()]: [rule.ruletype] [rule.name] spent [rule.cost + rule_threat]"
		if(rule.flags & HIGHLANDER_RULESET)
			highlander_executed = TRUE
...
/datum/game_mode/dynamic/process()
	...
	if(event_injection_cooldown < world.time)
		...
		event_injection_cooldown = (rand(GLOB.dynamic_event_delay_min, GLOB.dynamic_event_delay_max) + world.time)
	...
	if(threat_injection_cooldown < world.time)
		threat_injection_cooldown = (GLOB.dynamic_threat_delay + world.time)
		var/threat_to_add = GLOB.dynamic_chaos_level//Simple
		create_threat(threat_to_add)
